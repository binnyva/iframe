...
function buildDropDownArray($array, $name, $selected="", $extra=array()) {
	...
	print getBeginTag("select",array('name'=>$name,'id'=>$name)+$extra);
	foreach ($array as $key=>$value) {
		$attrbs = array('value'=>$key);
		if($key == $selected) $attrbs['selected']="selected";
		buildTag('option',$attrbs,$value);
	}
	print getEndTag("select");
}
...
function buildInput($name, $title='', $type='text', $data='', $extra=array(), $info='') {
	...
	print "<label for='$all_attributes[id]'>$title</label>" . getTag( $tag, $all_attributes, $value ) . $info . "<br />\n";
}
...
function getTag( $tag, $attributes, $value='') {
	...
	if($self_close) {
		$tag = getBeginTag( $tag, $attributes, true);
	} else {
		$tag = getBeginTag( $tag, $attributes ) . $value . getEndTag($tag);
	}
	return $tag;
}

function buildTag( $tag, $attributes, $value ) {
	print getTag( $tag, $attributes, $value );
}
